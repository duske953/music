...
async function getImg(model) {
  ...
  try {
    const imgResponse = model.map(
      async (ele) => await axios.get(`${ele}?apikey=${process.env.API_KEY}`)
    );
    // using "promise.all" to get all the images once
    const imgData = await Promise.all(imgResponse);
    return imgData.map((el, i) => el.data).map((el) => el.images);
  } catch (err) {
    throw err;
  }
}
...
module.exports = {
  ...
  async getMusicData() {
    try {
      ...
      const response2 = axios.get(
        `${process.env.API_URL}=${process.API_KEY}&range=day`
      );
      const [musicResponse1, musicResponse2] = await Promise.all([
        response1,
        response2,
      ]);
      ...
      modelState.albumsDetails = modelFunc.getAlbums(
        data,
        await albumsImg(getImageLink(data))
      );

      const data2 = await musicResponse2;
      modelState.albumTopDetails = modelFunc.getAlbums(
        data2,
        await albumsImg(getImageLink(data2))
      );
    } catch (err) {
      throw err;
    }
  ...
  async searchMusic(query) {
    ...
    try {
      const responseResult = await axios.get(
        `https://api.napster.com/v2.2/search/verbose?apikey=${process.env.API_KEY}&query=${query}`
      );
      ...
      modelState.searchResults[0].albums = [albums].map((el) =>
        modelFunc.getSearchData(el)
      );
      //data to get the artist search results
      modelState.searchResults[0].artists = [artist].map((el) =>
        modelFunc.getSearchData(el)
      );
    } catch (err) {
      throw err;
  ...
  async searchInfo(type, id, top, limit) {
    ...
    try {
      ...
      const responseSearchDetail = await axios.get(
        `http://api.napster.com/v2.2/${type}/${id}?apikey=${process.env.API_KEY}`
      );
      ...
      const responseTrack = await axios.get(
        `http://api.napster.com/v2.2/${type}/${
          modelState.musicInfo[0].id
        }/tracks${top ? `/top` : ""}?apikey=${process.env.API_KEY}${
          limit ? `&limit=10` : ""
        }`
      );
